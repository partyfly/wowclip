...
`edl.json` uses `wowclip.timeline.v1` and is the canonical editable document. Legacy `timeline.json` may be imported for migration, but new edits should be written back to `edl.json`.
